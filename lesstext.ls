...
# https://github.com/tka/serve/blob/master/lib/serve/view_helpers.rb#L542-L786
# Thanks gugod, hlb, tkalu and every original authors.
...
LoremHelpers = let
  ...
  h =
    ...
    lorem-words:
      (total, r) ->
        | r isnt undefined => r
        | otherwise        => [1 to total]map(-> randm WORDS)join(' ')
    lorem-sentence:
      (r) -> h.lorem-sentences 1, r
    lorem-sentences:
      (total, r) ->
        ...
        | otherwise
          [1 to total]
            .map -> capitalize h.lorem-words randm [4 to 15]
            .join '. '
    lorem-paragraph:
      (r) -> h.lorem-paragraphs 1, r
    lorem-paragraphs:
      (total, r) ->
        | r isnt undefined => r
        | otherwise
          [1 to total]
            .map -> h.lorem-sentences randm([3 to 7]), r
            .join "\n\n"
    lorem-name:
      (r) ->
        | r isnt undefined => r
        | otherwise   => "#{h.lorem-first-name!} #{h.lorem-last-name!}"
    lorem-first-name:
      (r) ->
        | r isnt undefined => r
        | otherwise   => FIRST_NAMES[rand FIRST_NAMES.length]
    lorem-last-name:
      (r) ->
        | r isnt undefined => r
        | otherwise   => LAST_NAMES[rand LAST_NAMES.length]
    lorem-email:
      (r) ->
        | r isnt undefined => r
        | otherwise
...
JpLoremHelpers = let
  ...
  h =
    ...
    lorem-name-en:
      (r) ->
        | r         => r
        | otherwise => "#{h.lorem-first-name-en!} #{h.lorem-last-name-en!}"
    lorem-first-name:
      (r) ->
        | r         => r
        | otherwise => randm FIRST_NAME
    lorem-first-name-en:
      (r) ->
        | r         => r
        | otherwise => randm FIRST_NAME_EN
    lorem-last-name:
      (r) ->
        | r         => r
        | otherwise => randm LAST_NAME
    lorem-last-name-en:
      (r) ->
        | r         => r
        | otherwise => randm LAST_NAME_EN
    lorem-email:
      (r) ->
        | r         => r
        | otherwise
          username =
            h.lorem-first-name-en! + randm(DELIMITERS) + h.lorem-last-name-en!
          "#{username.toLowerCase!}@#{randm DOMAINS}"
    lorem-word:
      (r) ->
        | r         => r
